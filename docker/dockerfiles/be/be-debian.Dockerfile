# This docker file build the Starrocks be debian image
...
#     > DOCKER_BUILDKIT=1 docker build --build-arg ARTIFACT_SOURCE=local --build-arg LOCAL_REPO_PATH=. -f docker/dockerfiles/be/be-debian.Dockerfile -t be-debian:latest .
#
...
RUN apt-get update -y && apt-get install -y --no-install-recommends \
        binutils-dev default-jdk python2 default-mysql-client curl vim tree net-tools less tzdata linux-perf && \
